...
// Suivre si le formulaire a été modifié
const isFormDirty = ref(false);

// Vérifier les modifications du formulaire
...
const submit = () => {
    if (isFormValid.value) {
        // Mettre à jour d'abord la question
        form.put(route('admin.questions.update', props.question.id), {
            onSuccess: () => {
                // Ensuite, mettre à jour les réponses
                updateAnswers();
            },
        });
    }
...
const updateAnswers = async () => {
    try {
        const response = await axios.post(
            route('admin.questions.answers.manage', props.question.id),
...
// Gérer la navigation avant de quitter la page
window.onbeforeunload = (e) => {
    if (isFormDirty.value) {
        e.preventDefault();
        // La plupart des navigateux ne permettent plus de personnaliser ce message
        e.returnValue = '';
    }
};
</script>
...
<template>
    <Head :title="`Éditer la question #${question.id}`" />
    
    <AdminLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                    Éditer la question #{{ question.id }}
                </h2>
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <div class="flex space-x-3">
                                        <button
                                            type="button"
                                            @click="router.reload()"
                                            class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:hover:bg-gray-600"
                                        ...
                                        <button
                                            type="submit"
                                            :disabled="form.processing || !isFormValid || !isFormDirty"
